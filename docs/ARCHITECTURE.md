...
| D20 | Agenda | ~~**react-big-calendar** (MIT) estilizado, com visão por mecânico e arrastar e soltar~~ **Revista na E8: ver D28** | FullCalendar | No FullCalendar, as visões **por recurso** (coluna por mecânico) são pagas. A própria D20 já previa a saída que a E8 acabou tomando: "se a customização ficar cara, a visão de dia/semana vira componente próprio" |
| D21 | Landing page ✅ E17 | **Astro** (HTML estático) em `apps/landing`, com os tokens do painel e zero JS de runtime | Mesma SPA React | **Única troca de stack proposta.** A landing precisa de SEO e de carregar instantaneamente; uma SPA entrega um HTML vazio ao Google. O Astro aceita componentes React, então os blocos visuais são reaproveitados |
...
| D30 | Conta a receber × OS (E13) | A conta **espelha** a OS: valor = total aprovado, e a baixa é o pagamento do caixa (E7), gravado em `payments` | Lançamento financeiro independente, com baixa própria | Com duas entradas para o mesmo dinheiro, "recebido" viraria dois números que divergem no primeiro fiado. Espelhando, a tela do financeiro registra o pagamento na OS e as parcelas quitam da mais velha para a mais nova. Quem mantém a sincronia é o `applyWorkOrderChange`, por onde **toda** gravação da OS passa — ninguém precisa lembrar de sincronizar nada |
| D31 | "Vencida" não é situação gravada (E13) | `OPEN`, `PARTIAL`, `PAID` e `CANCELED` no banco; vencida = `due_date < hoje` com saldo, calculado na leitura | Status `OVERDUE` gravado | Gravar exigiria um job varrendo a tabela toda madrugada só para o selo da tela ficar certo — e ele ficaria errado entre a meia-noite e o job. É a mesma escolha da cotação vencida (E11). "Hoje" é o dia da **oficina**, não o do servidor |
| D32 | Idempotência de dinheiro (E13) | `client_request_id` (UUID da tela) com UNIQUE por oficina em `payments` e `financial_settlements`: o segundo POST devolve o estado atual, sem cobrar de novo | Só travar o botão no front | Rede ruim de oficina repete POST sem ninguém clicar duas vezes. A regra que já valia para o recebimento de compra (E12) passou a valer para todo dinheiro |
| D33 | Providers de pesquisa de peça (E14) | Todos os providers de hoje leem o banco da própria oficina e rodam **em sequência**, dentro da transação | Rodar em paralelo, como a Fase 0 previa | Eles dividem uma conexão (a da transação) e o Postgres não multiplexa consulta na mesma conexão. O paralelismo com timeout continua sendo o desenho para provider EXTERNO, que não usa o `tx`. Um provider que falha vira "indisponível" e não derruba a busca |
